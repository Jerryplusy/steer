...
CS_PROMPT = """你是一个严格的评分员。请评估「模型的回答」是否体现了「目标概念」，只看概念是否被融入，不管语法或逻辑连贯性。
...
只输出一个 0-2 之间的整数，不要任何解释。
分数："""

IS_PROMPT = """你是一个严格的评分员。请评估「模型的回答」与「用户指令」的相关程度，只看主题相关度，不管语法、连贯或信息量。
...
FS_PROMPT = """你是一个严格的评分员。请评估「模型的回答」的流畅度，只看语言本身，不管内容是否完整、相关或有信息量。

评分标准（0-2 分）：
- 0 分：不流畅、高度不自然（如不可读或重复）
- 1 分：基本流畅但有明显错误或生硬
- 2 分：流畅、近乎完美
